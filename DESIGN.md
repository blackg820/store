...
This project uses Shopify as a quality and interaction reference, but the production storefront should be light-first unless a task explicitly asks for dark mode. Apply the same Shopify-inspired principles across customer-facing store pages: calm premium surfaces, generous spacing, light display typography, full-pill actions, subtle green accents, polished product cards, responsive mobile layouts, and clean loading/empty states. Do not convert storefront pages to dark mode by default.
